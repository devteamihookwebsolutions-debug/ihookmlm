    ...
    public static function showProductList($records)
    {
        ...
        if (count((array)$records) > 0) {
            for ($i = 0;$i < count((array)$records);$i++) {
                $output .= '<option value="' . $records[$i]->id . '">' . $records[$i]->name . ' </option>';
            }
        }
        $output .= '</select>';
        return $output;
    ...
    public static function editEshopProducts($records, $eshop_id)
    {
        ...
        if (count((array)$records) > 0) {
            for ($i = 0;$i < count((array)$records);$i++) {
                ...
                $output .= '<option value="' . $records[$i]->id . '" '.$selected.'>' . $records[$i]->name . ' </option>';
            }
        }
        $output .= '</select>';
        return $output;
