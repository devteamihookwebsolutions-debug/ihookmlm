...
public static function getUserCommissionReports(Request $request)
{
    ...
    $historyTypes = HistoryType::from($prefix.'_history_type_table')
    ...
    //    dd($historyTypes);
    // Base query
    $query = Reports::from($prefix.'_matrix_members_link_table as a')
        ->join($prefix.'_members_table as b', 'a.members_id', '=', 'b.members_id')
        ->join($prefix.'_history_table as c', 'b.members_id', '=', 'c.history_member_id')
    ...
    $totalCount = Reports::from($prefix.'_matrix_members_link_table as a')
        ->join($prefix.'_members_table as b', 'a.members_id', '=', 'b.members_id')
        ->join($prefix.'_history_table as c', 'b.members_id', '=', 'c.history_member_id')
        ->whereIn('c.history_type', $historyTypes)
        ->where('c.history_amount', '!=', 0)
