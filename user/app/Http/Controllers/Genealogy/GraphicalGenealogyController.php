...
public function viewTree($encrypted, Request $request)
{
    ...
    // Block matrix_type_id 6
    if ($matrix_type_id == 6) {
        abort(403, 'Access denied for this matrix type');
    }

    // Determine genealogy type
    ...
    // Template selection (kept as in your code)
    ...
    if ($matrix_type_id == 1) {
        ...
        // Handle different genealogy types for Binary
        if ($type === 'advancedgenealogy' || $type === 'directdownlinegenealogy') {
            $output['genealogy'] = MGraphicalGenealogy::updateGenealogyDetails($members_id, $matrix_id);

            $blade = $type === 'advancedgenealogy'
                ? 'user::genealogy.graphical'
                : 'user::genealogy.binary_graphical_directdownline';
        ...
        if (in_array($type, ['grpgenealogy', 'countgenealogy', 'directdownlinegenealogy', 'advancedgenealogy'])) {
            ...
            $output['matrix_name'] = $matrix_name;
