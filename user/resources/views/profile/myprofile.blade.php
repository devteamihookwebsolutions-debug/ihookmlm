    ...
    <div
      ...
      <div
          ...
          <div class="border border-neutral-300 rounded-lg p-6">
            ...
            <div class="widget-data">
              ...
              <div class="text-2xl font-semibold dark:text-white"><span
                  class="text-sm top-n-4">{{ session('site_settings.site_currency') }}</span><span
        ...
        <div class="mb-5 max-h-[400px] overflow-y-auto
          ...
          <ul class="flex flex-col space-y-4 text-sm font-medium text-black dark:text-white md:me-4 mb-4 md:mb-0 m-5">
            <li>
      ...
      <div class="col-span-3">
        ...
        <div class="w-full">
          <div id="profile" class="tab-content">
            <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-2 gap-5 m-0">
...
>{{ __('Save Changes') }}</button>
              ...
              <div class="content-center relative bg-white  dark:bg-neutral-900 rounded-lg m-auto px-2 pb-2 py-2 place-items-center items-center space-x-6 mt-6">
                <div class="m-auto relative table">
                  <img id='preview_img' class="h-40 w-40 rounded-full p-1"
                    src="{{$_ENV['CDNCLOUDEXTURL']}}/['members_image'] }}" alt="Current profile photo"
  ...
  <div id="transaction" class="tab-content hidden">
    ...
    <div class="grid grid-cols-1 gap-5 mb-5">
        <form id="showMyTransaccPass"
              action="{{ route('user.profile.update.transaction.password') }}"
              method="POST">
            @csrf

            <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm p-6 mb-5 border border-neutral-200 dark:border-neutral-700">

                <h3 class="text-lg font-semibold mb-6 text-black dark:text-white">
                    {{ __('Transaction Password') }}
                </h3>

                <!-- CURRENT PASSWORD – Only if already set -->
                @if($has_transaction_password ?? false)
                    <div class="mb-6">
                        <label for="currenttransactionpassword"
                               class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                            {{ __('Current Transaction Password') }}
                        </label>
                        <input type="password"
                               id="currenttransactionpassword"
                               name="currenttransactionpassword"
                                 placeholder="{{ __('Enter current transaction password') }}"
                               class="w-full px-4 py-2.5 border border-gray-300 dark:border-neutral-600 rounded-lg
                                      focus:ring-2 focus:ring-neutral-500 focus:border-neutral-500
                                      bg-white dark:bg-neutral-800 text-black dark:text-white"
                               required>
                        @error('currenttransactionpassword')
                ...
                <div class="mb-6">
                    <label for="transactionpassword"
                           class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                        {{ $has_transaction_password ?? false ? __('New Transaction Password') : __('Transaction Password') }}
                    </label>
                    <input type="password"
                           id="transactionpassword"
                           name="transactionpassword"
                           placeholder="{{ __('Enter new transaction password') }}"
                           class="w-full px-4 py-2.5 border border-gray-300 dark:border-neutral-600 rounded-lg
                                  focus:ring-2 focus:ring-neutral-500 focus:border-neutral-500
                                  bg-white dark:bg-neutral-800 text-black dark:text-white"
                           required>
                ...
                <!-- CONFIRM PASSWORD -->
                <div class="mb-6">
                    <label for="transactionpassword_confirmation"
                           class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                        {{ __('Confirm New Transaction Password') }}
                    </label>
                    <input type="password"
                           id="transactionpassword_confirmation"
                           name="transactionpassword_confirmation"
                ...
                <!-- BUTTONS -->
                <div class="flex justify-end gap-4 pt-4 border-t border-gray-200 dark:border-neutral-700">
                    <button type="button"
                            onclick="window.location.reload()"
                            class="px-6 py-2.5 text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-neutral-800
                                   hover:bg-gray-200 dark:hover:bg-neutral-700 rounded-lg transition">
                        {{ __('Cancel') }}
                    </button>
                    <button type="submit"
                            class="px-7 py-2.5 bg-neutral-800 dark:bg-neutral-100 text-white dark:text-black
                                   font-medium rounded-lg hover:scale-105 transition shadow-md">
                        {{ __('Save Changes') }}
                    </button>
                </div>
            </div>
...
@include('user::profile.components.myprofile_scripts')
@endsection
