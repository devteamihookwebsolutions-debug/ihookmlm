...
/**
 * This class contains public functions related to DPayouts
 *
 * @package         DPayouts
 * @category        Display
...
class DPayouts
{
    ...
    foreach ($account_details as $key => $value) {
        ...
        if (!empty($paymentsettings_default_name)) {
            ...
            $viewaccount_details .= ucfirst(str_replace('_', ' ', $key)) . ' : ' . $value . '<br/>';
                ...
                // Wallet type
                $wallet_type = DB::table($prefix .'_wallettype')
                    ->where('wallet_type_id', $record->history_wallet_type)
                    ->value('wallet_name') ?? "-";

                // Withdraw commission
                $adminwithdrawcommission = DB::table($prefix .'_generalsettings_table')
    ...
    public static function getMembersAccountDetails($where)
    {
        ...
        return DB::table($prefix .'_members_accounts_table')
            ->where($column, $value)
            ->get();
    }
