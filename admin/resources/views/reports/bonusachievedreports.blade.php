            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-1 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Bonus Achieved
                       </span>
                    </div>
                </li>
            </ol>
...
<th class="px-4 py-3  dark:text-white bg-white dark:bg-gray-900"> <span class="flex items-center">SNo
...
<th class="px-4 py-3  dark:text-white bg-white dark:bg-gray-900"><span class="flex items-center">Bonus Name
<svg class="w-4 h-4 ms-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m8 15 4 4 4-4m0-6-4-4-4 4"/>
                                            </svg>
                                        </span>
</th>
<th class="px-4 py-3  dark:text-white bg-white dark:bg-gray-900"><span class="flex items-center">Bonus Type
<svg class="w-4 h-4 ms-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m8 15 4 4 4-4m0-6-4-4-4 4"/>
                                            </svg>
                                        </span>
</th>
<th class="px-4 py-3  dark:text-white bg-white dark:bg-gray-900"><span class="flex items-center">Bonus Result
<svg class="w-4 h-4 ms-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m8 15 4 4 4-4m0-6-4-4-4 4"/>
                                            </svg>
                                        </span>
</th>
<th class="px-4 py-3  dark:text-white bg-white dark:bg-gray-900"><span class="flex items-center">Status
<svg class="w-4 h-4 ms-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m8 15 4 4 4-4m0-6-4-4-4 4"/>
                                            </svg>
                                        </span>
</th>
<th class="px-4 py-3  dark:text-white bg-white dark:bg-gray-900"><span class="flex items-center">Achieved Date
<svg class="w-4 h-4 ms-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m8 15 4 4 4-4m0-6-4-4-4 4"/>
                                            </svg>
                                        </span>
...
<script>
    const getBonusDataUrl = "{{ route('admin.getBonusData') }}";
</script>
...
let csvContent = 'SNo, Username, Bonus Name, Bonus Type, Bonus Result, Status, Achieved Date\n'; // Set headers
...
link.download = 'bonusachieved.csv'; // Set the file name
link.click();
});
 ...
 document.getElementById('export-pdf').addEventListener('click', function(event) {
        ...
        doc.autoTable({
            head: [['SNo', 'Username', 'Bonus Name', 'Bonus Type', 'Bonus Result','Status','Achieved Date']], // Headers
            body: tableData,
        });

        doc.save('bonusachieved.pdf'); // Download PDF file
    });
    document.getElementById('print-table').addEventListener('click', function(event) {
        ...
        printWindow.document.write('<html><head><title>Bonus Achieved</title>');
        printWindow.document.write('<style>');
        printWindow.document.write('table { width: 100%; border-collapse: collapse; }');
        printWindow.document.write('th, td { border: 1px solid black; padding: 8px; text-align: left; }');
        printWindow.document.write('</style></head><body>');
        printWindow.document.write('<h1>Bonus Achieved</h1>');
...
const showAllRows = () => {
    ...
    rows.forEach(row => {
        if (!row.id || row.id !== "no-records-message") {
            row.style.display = "";
        }
    });
...
const filterRows = (searchQuery) => {
    ...
    if (searchQuery.trim() === "") {
        ...
        paginationContainer.style.display = 'block'; // Show pagination when search is empty
        const noRecordsMessage = document.getElementById("no-records-message");
        if (noRecordsMessage) noRecordsMessage.remove();
        return;
    }
    ...
    if (!foundMatch) {
        ...
        if (noRecordsMessage) noRecordsMessage.remove();
        paginationContainer.style.display = 'none'; // Hide during active search
    }
};
...
const fetchRecords = async (page = 1, query = lastSearchQuery, dataColumns = null) => {
    try {
        ...
        const response = await fetch(`${getBonusDataUrl}?${queryParams.toString()}`);
        const data = await response.json();
        if (!data.records || data.records.length === 0) {
            tableBody.innerHTML = "<tr id='no-data-row'><td colspan='7' class='px-6 py-4 text-center border-b border-gray-300 text-sm text-black'>No records found</td></tr>";
               createFilterRow();
            return;
        }
        ...
        <tr class="border-b border-gray-300 block md:table-row">
                    ...
                    <td class="font-medium  whitespace-nowrap dark:text-white">${record.bonusname}</td>
                    <td class="font-medium  whitespace-nowrap dark:text-white">${record.bonustype}</td>
                    <td class="font-medium  whitespace-nowrap dark:text-white">${record.amount}</td>
                    <td class="font-medium  whitespace-nowrap dark:text-white">${record.status}</td>
                    <td class="font-medium  whitespace-nowrap dark:text-white">${record.formatdate}</td>
                    <td class="font-medium whitespace-nowrap dark:text-white">${record.action}</td>
                </tr>
        `;
        });
   // ✅ Update total records info here
   if (datatableInfo) {
