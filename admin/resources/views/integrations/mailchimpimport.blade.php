...
<main class="flex-grow">
    <div class="space-y-6">
        ...
        <div class="flex p-4 mb-6 text-xs text-blue-800 rounded-lg bg-neutral-50 dark:bg-neutral-900 dark:text-blue-400 border border-blue-300" role="alert">
            ...
            <div>
                <p class="font-medium">MailChimp Contact Import</p>
                <p class="mt-1">
                    Import selected customers/members into your MailChimp audience for better email marketing automation and campaigns.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6 dark:border-neutral-700 dark:bg-neutral-900 dark:text-white border border-neutral-200">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
                    Members Ready for MailChimp Import
                </h2>
            </div>

            <form action="{{ route('admin.integration.mailchimp.import') }}" method="POST">
                @csrf

                @if($mailchimpcontacts->isEmpty())
                    <div class="p-10 text-center bg-gray-50 dark:bg-neutral-800 rounded-lg border border-dashed border-gray-300 dark:border-neutral-600">
                        ...
                        <h3 class="mt-3 text-lg font-medium text-gray-900 dark:text-gray-200">
                            No members waiting for import
                        </h3>
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            All members are already synced with MailChimp or there are no records available.
                        </p>
                    </div>
                @else
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 dark:bg-neutral-900 dark:divide-neutral-700">
                                @foreach($mailchimpcontacts as $member)
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="flex flex-wrap gap-3">
                            <button type="button" id="select-all-btn"
                                    class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-neutral-800 dark:border-neutral-600 dark:text-gray-300 dark:hover:bg-neutral-700 transition-colors">
                                Select All
                            </button>
                        ...
                        <button type="submit" id="import-btn"
                                class="px-6 py-2.5 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
                                disabled>
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                            </svg>
                            Import Selected to MailChimp
                        </button>
                    </div>
                @endif
            </form>
