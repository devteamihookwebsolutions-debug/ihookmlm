...
/**
 * This class contains public functions related to DMatrixPackage
 *
 * @package         DMatrixPackage
 * @category        Display
...
class DMatrixPackage
{

    public static function showPackageDetails($records, $Err)
{
    ...
    // Determine payment method and duration
    $package_paymentmethod = "Unknown";
    $package_duration = "-";
    $pack_payment_fields = [];

    if (!empty($record['pack_payment_fields'])) {
        $data = @unserialize($record['pack_payment_fields']);
        if ($data !== false || $record['pack_payment_fields'] === 'b:0;') {
            $pack_payment_fields = $data;
        }
    }
    ...
    if ($record['package_paymentmethod'] == 0) {
        ...
        if (!empty($pack_payment_fields)) {
            if (isset($pack_payment_fields['interval_selector']) && $pack_payment_fields['interval_selector'] === "custom") {
        ...
        $package_duration = $record['package_duration'] . " Days";
        if (!empty($pack_payment_fields) && isset($pack_payment_fields['interval_selector'])) {
            $package_duration .= " & Every " . $pack_payment_fields['interval_selector'];
        }
    }
    ...
    $output .= '<tr>
        ...
        <td>' . $record['package_direct_commission'] . '</td>
        <td>' . $record['package_direct_commission_method'] . '</td>
        <td>' . $record['package_pv'] . '</td>
        <td class="flex gap-2">
            <a aria-label="link" href="javascript:void(0);" onclick="showEditPackage(' . $record['package_id'] . ')">
            ...
            <a aria-label="link" href="javascript:void(0);" onclick="deletePackage(' . $record['package_id'] . ')" title="' . __('Delete') . '">
                <svg class="w-6 h-6 text-black dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                    <path fill-rule="evenodd" d="M8.586 2.586A2 2 0 0 1 10 2h4a2 2 0 0 1 2 2v2h3a1 1 0 1 1 0 2v12a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V8a1 1 0 0 1 0-2h3V4a2 2 0 0 1 .586-1.414ZM10 6h4V4h-4v2Zm1 4a1 1 0 1 0-2 0v8a1 1 0 1 0 2 0v-8Zm4 0a1 1 0 1 0-2 0v8a1 1 0 1 0 2 0v-8Z" clip-rule="evenodd"/>
                </svg>
            </a>

            <a aria-label="link" data-toggle="modal" data-target="#package_commission_status" onclick="setPackageLevelCommission(' . $record['package_id'] . ')" title="' . __('Set level commision') . '">
                <svg class="w-6 h-6 text-black dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
