   ...
   <div class="flex mb-4" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-1 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Fund Transfer</span>
                    </div>
                </li>
            </ol>
      </div>
        ...
        <main class="flex-grow">
            <div class="container mx-auto px-4 sm:px-6 lg:px-0 py-6 lg:py-3">
                ...
                <div class="flex p-4 mb-6 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 border border-blue-300"
                    role="alert">
                    ...
                    <div>
                        <div>
                            <p class="mb-2">Here you can check the status of transfer funds from one members commission
                                account to another members commission account.</p>
                        </div>
                    </div>
                </div>
...
<script>
  const getFundtransferRecordDataUrl = "{{ route('admin.fundtransferdata') }}";
</script>
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const tableBody = document.querySelector("#data-table tbody");
        const getFundtransferRecordDataUrl = "{{ route('admin.fundtransferdata') }}"; // ✅ your API endpoint

        // Fetch Ewallet Data
        fetch(getFundtransferRecordDataUrl)
                ...
                if (data && data.data && data.data.length > 0) {
                    ...
                    const rows = data.data.map(row => `
                        <tr class="border-b dark:border-gray-700">
                            <td class="px-4 py-2">${row.from_user ?? '-'}</td>
                            <td class="px-4 py-2">${row.to_user ?? '-'}</td>
                            <td class="px-4 py-2">${row.amount ?? '-'}</td>
                            <td class="px-4 py-2">${row.wallet ?? '-'}</td>
            ...
            .catch(error => {
                console.error("Error fetching Ewallet data:", error);
                tableBody.innerHTML = `
                    <tr>
                        <td colspan="7" class="text-center py-3 text-red-500">Failed to load data</td>
                    </tr>`;
            });
    });
</script>
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const tableBody = document.querySelector("#data-table tbody");
        const getEwalletRecordDataUrl = "{{ route('admin.ewalletGetData') }}";
        ...
        function exportJSON() {
            ...
            downloadFile("export.json", JSON.stringify(json, null, 2), "application/json");
        }

        function exportTXT() {
            const { headers, rows } = getTableData();
            const txt = [headers.join("\t"), ...rows.map(r => r.join("\t"))].join("\n");
            downloadFile("export.txt", txt, "text/plain");
        }
        ...
        function exportSQL() {
            ...
            const sql = rows.map(r =>
                `INSERT INTO ${tableName} (${headers.join(", ")}) VALUES (${r.map(v => `'${v.replace(/'/g, "''")}'`).join(", ")});`
            ).join("\n");
            downloadFile("export.sql", sql, "text/sql");
        }

        // --- Fix dropdown behavior ---
        const exportButtons = ["export-csv", "export-json", "export-txt", "export-sql"];
        exportButtons.forEach(id => {
            const btn = document.getElementById(id);
            btn.addEventListener("click", (e) => {
                e.preventDefault();
                e.stopPropagation(); // prevent Flowbite from auto-closing before export

                // Run export based on button ID
                switch (id) {
                ...
                // Safely close dropdown AFTER export completes
                setTimeout(() => dropdown.hide(), 100);
            });
        });
