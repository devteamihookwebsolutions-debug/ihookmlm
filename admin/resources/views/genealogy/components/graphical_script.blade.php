...
<script type="text/javascript">
    ...
    function callHandler(nodeId) {
        ...
        // If node has a real member (name exists and not empty)
        if (nodeData.name && nodeData.name !== '') {
            window.location.href = "{{ $_ENV['BCPATH'] }}/memberarea/show/" + nodeId;
        } else {
            // Empty slot → open registration with sponsor = parent, position = node's position
            // Adjust this URL according to your actual registration route
            var registerUrl = "{{ $_ENV['FCPATH'] }}/register/" +
                              (nodeData.pid || '') + "/" +
                              (nodeData.position || 'left');
