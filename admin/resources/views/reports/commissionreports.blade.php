            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-1 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Total Commission
                       </span>
                    </div>
                </li>
            </ol>
                        ...
                        <div id="table-container" >
                            <table id="filter-table" class="min-w-full mt-5 divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr
                                        class="border-b border-gray-300 block md:table-row text-gray-500 text-xs uppercase">
                                        ...
                                        <th class="px-4 py-3"><span class="flex items-center">Type<svg
                                                    class="w-4 h-4 ms-1" aria-hidden="true"
                                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    fill="none" viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round"
                                        ...
                                        <th class="px-4 py-3"><span class="flex items-center">Date<svg
                                                    class="w-4 h-4 ms-1" aria-hidden="true"
                                                    xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    fill="none" viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round"
...
<script>
    const getCommissionDataUrl = "{{ route('admin.getCommissionData') }}";
</script>
...
let lastSearchQuery = '';
let lastColumnIndex = null; // Track which column was searched
let debounceTimer;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('print-table').addEventListener('click', function(event) {
        event.preventDefault();
        const rows = document.querySelectorAll('#filter-table tbody tr');
        let printContent = '<table border="1" cellpadding="5" cellspacing="0" style="width:100%; border-collapse: collapse;">';
        printContent += '<thead><tr><th>Sno</th><th>Username</th><th>Amount</th><th>Type</th><th>Date</th></tr></thead><tbody>';
        rows.forEach(row => {
        ...
        printWindow.document.write('<html><head><title>Commissions</title>');
        ...
        printWindow.document.write('<h1>Commissions</h1>');
        printContent;
...
const createFilterRow = () => {
    ...
    tr.className = 'search-filtering-row text-black dark:text-white bg-white dark:bg-neutral-900';
    ...
    Array.from(headerCells).forEach((th, index) => {
        ...
        // Skip filter for Sno (0) and Amount (2) - adjust as needed
        if (index === 0 || index === 2) {
...
const fetchRecords = async (page = currentPage) => {
    try {
        ...
        const response = await fetch(`${getCommissionDataUrl}?${queryParams.toString()}`);
        ...
        records.forEach((record) => {
            tableBody.innerHTML += `
                <tr class="border-b border-gray-300 block md:table-row">
                    <td class="px-4 py-3 text-sm">${record.sno ?? '-'}</td>
        ...
        // Refresh filter row inputs (preserve values)
        createFilterRow();
        // Re-apply last search query if exists
        if (lastSearchQuery && lastColumnIndex !== null) {
            const inputs = document.querySelectorAll('.search-filtering-row input');
            if (inputs[lastColumnIndex]) {
                inputs[lastColumnIndex].value = lastSearchQuery;
            }
        }
...
const renderPagination = (totalRecords, currentPage) => {
    ...
    if (currentPage > 1) {
        ...
        prev.className = 'pagination-button px-4 py-2 mx-1 border rounded';
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        btn.textContent = i;
        btn.className = 'pagination-button px-4 py-2 mx-1 border rounded';
        if (i === currentPage) {
            btn.disabled = true;
    ...
    if (currentPage < totalPages) {
        ...
        next.className = 'pagination-button px-4 py-2 mx-1 border rounded';
