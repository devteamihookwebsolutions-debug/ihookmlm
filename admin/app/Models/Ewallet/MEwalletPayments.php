...
/**
 * This class contains public functions related to MEwalletPayments
 *
 * @package         MEwalletPayments
 * @category        Model
...
public static function showEwalletManagement()
{
    ...
    if (!empty($search)) {
        $query->where(function ($q) use ($search) {
            $q->where('paymenthistory_trans_id', 'like', "%{$search}%")
              ->orWhere('paymenthistory_mode', 'like', "%{$search}%")
              ->orWhere('paymenthistory_status', 'like', "%{$search}%")
              ->orWhere('paymenthistory_amount', 'like', "%{$search}%")
              ->orWhere('paymenthistory_date', 'like', "%{$search}%");
        });
    }
    ...
    $iTotal = DB::table($tableName)
        ->whereIn('paymenthistory_type', ['ewalletcredits'])
        ->count();

    // Convert paginator to array of data (if old function expects plain array)
    $recordsArray = $records->items();
    // dd($recordsArray);
    // Return to your existing function
    return DEwalletPayments::showEwalletManagement($recordsArray, $iTotal);
}
