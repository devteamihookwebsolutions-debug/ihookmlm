    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-1 rtl:space-x-reverse">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Add Matching Bonus</span>
            </div>
        </li>
    </ol>
</div>
        ...
        <div>
            <div class="p-4 rounded-lg">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5">
                    <form action="{{ route('matchbonus.store') }}" method="POST" id="matchbonus"
                        ...
                        <div class="mb-5 lg:w-1/2">
                            ...
                            <select name="matrix_id" id="matrix_id"
                                ...
                                onchange="showMatchingBonus(this.value);">
                        ...
                        @if($sub1 != '')
                        <div id="hide_element" class="mb-5 lg:w-1/2">
                            <label
                                class="block mb-3 text-xs text-gray-600 dark:text-gray-300">{{ __('Matching Bonus Name') }}</label>
                            <input type="text" id="matchingbonus_name" name="matchingbonus_name" required
                                class="bg-gray-50 border border-gray-300 text-gray-600 text-xs rounded-lg block w-full p-2 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300"
                        ...
                        <input aria-label="label" type="hidden" name="levelcount" id="levelcount" value="1">
                        <div class="mb-5 lg:w-1/2">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="px-6 text-right">
                                            <div class="flex items-center p-2.5">
                                                <span
                                                    class="text-sm font-medium text-black dark:text-white">{{ __('Level') }}</span>
                                                ...
                                                <span
                                                    class="text-sm font-medium text-black dark:text-white">{{ __('Rank') }}</span>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
                        ...
                        <div id="dvPassport" class="mb-5">
                            <div class="flex justify-end items-center" id="addbox">
                                <ul class="flex space-x-2">
                                    <li>
                                        <button type="button" id="btn2"
                                            class="px-4 py-2 bg-gray-800 text-xs text-white hover:bg-gray-900 rounded-lg dark:bg-blue-500 dark:hover:bg-blue-600">{{ __('Add') }}</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
                        <div id="dvPassport1" class="hidden mb-5">
                        ...
                        <div class="col-span-2 relative overflow-x-auto bg-white dark:bg-gray-900 rounded-lg shadow p-2"
                            id="addlevelrank">
                        ...
                        <div class="col-span-2 relative overflow-x-auto bg-white dark:bg-gray-900 rounded-lg shadow p-2 hidden"
                            id="addrankmatch">
                        ...
                        <div class="flex justify-center pt-10">
                            <a href="javascript:void(0);" onclick="window.history.back();"
                             class="text-gray-900 bg-white border border-gray-300hover:bg-gray-100 rounded-lg text-xs px-4 py-2 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600">{{ __('Cancel') }}</a>
                            <button type="submit"
                                class="text-gray-900 bg-white border border-blue-300 hover:bg-blue-100 rounded-lg text-xs px-4 py-2 me-2 mb-2 dark:bg-blue-800 dark:text-white dark:border-blue-600 dark:hover:bg-blue-700 dark:hover:border-blue-600">{{ __('Submit') }}</button>
                        </div>
                        @endif
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    class FormHandler {
        ...
        initializeElements() {
            this.elements = {
                form: document.getElementById('matchbonus'),
                matrixIdSelect: document.getElementById('matrix_id'),
        ...
        attachEventListeners() {
            ...
            this.elements.matrixIdSelect?.addEventListener('change', () => {
                const matrixId = this.elements.matrixIdSelect.value;
                window.location.href = matrixId ?
                    `/admin/matchbonus/add/${matrixId}` :
                    '/admin/matchbonus/add';
            });
        ...
        addLevelRow() {
            const counter = this.elements.levelTable.rows.length + 1;
            const newRow = this.elements.levelTable.insertRow();
            newRow.id = `level_r${counter}`;

            newRow.innerHTML = `
                    <td class="p-3">
                        <input type="number" name="l_levels[]" value="${counter}" readonly required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                    </td>
                    <td>
                        <input type="number" min="0" step="0.01" name="l_commission[]" id="l${counter}_commission" required aria-describedby="l${counter}_commission-error" class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                    ...
                    <td>
                        <select name="l_commissionpercentage[]" required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                            <option value="1">{{ __('Paring Bonus') }}</option>
                            <option value="2">{{ __('Total Sales') }} ({{ __('MLM') }})</option>
                        </select>
                    </td>
                    <td>
                        <select name="l_method[]" required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                            <option value="1">%</option>
                        </select>
                    </td>
                    <td>
                        <select name="l_wallet[]" required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
        ...
        addRankRow() {
            const counter = this.elements.rankTable.rows.length + 1;
            const matrixId = this.elements.matrixIdSelect.value;

            if (!matrixId) {
                Swal.fire('Error', 'Please select a plan first.', 'error');
                return;
            }
            ...
            fetch("/admin/matchbonus/getrankgen", {
                ...
                .then(data => {
                    ...
                    newRow.innerHTML = `
                        <td class="p-3">
                            <input type="number" name="r_levels[]" value="${counter}" readonly required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                        </td>
                        <td>
                            <input type="number" min="0" step="0.01" name="r_commission[]" id="r${counter}_commission" required aria-describedby="r${counter}_commission-error" class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                        ...
                        <td>
                            <select name="r_commissionpercentage[]" required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                                <option value="1">{{ __('Paring Bonus') }}</option>
                                <option value="2">{{ __('Total Sales') }} ({{ __('MLM') }})</option>
                            </select>
                        </td>
                        <td>
                            <select name="r_method[]" required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                                <option value="1">%</option>
                            </select>
                        </td>
                        <td>
                            <select name="r_wallet[]" required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                                <option value="2">{{ __('E-wallet') }}</option>
                                <option value="1">{{ __('C-wallet') }}</option>
                            </select>
                        </td>
                        <td>
                            <select name="r_rank[]" required class="text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-900 dark:text-white border border-gray-200 dark:border-gray-800">
                                ${rankOptions}
                            </select>
                        </td>
                        <td class="delete-cell">
