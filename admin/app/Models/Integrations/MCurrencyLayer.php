<?php
/**
 * This class contains public static functions related to CurrencyLayer api.
 *
 * @package         MCurrencyLayer
 * @category        Model
...
use Query\Bin_Query;
use Model\Factories\MSiteFields;

class MCurrencyLayer {
    /**
     * This public static function is used  to updatecurrencylayer Twitter
    */
    public static function updatecurrencylayer() {
        if($_POST['configure']=='currencylayer'){
            MSiteFields::updateSiteFields('multicurrency_access_key');
        }
            ...
            if ($key != 'do' && $key != 'submit' && $key != 'action') {
                ...
                if ($query->executeQuery($sql_site)) {
                    $sql = "UPDATE " . $_ENV['PROMLM_PREFIX'] . "sitesettings_table
                        ...
                        sitesettings_description='currencylayerapi'";
                    $obj = new Bin_Query();
                    $obj->updateQuery($sql);
                } else {
                    foreach ($_POST as $key => $value) {
                        ...
                        if ($key != 'do' && $key != 'submit' && $key != 'action') {
                            ...
                            VALUES('" . $value . "','" . $key . "','currencylayerapi')";
                            $obj = new Bin_Query();
                            $obj->updateQuery($sql);
                        }
                    }
